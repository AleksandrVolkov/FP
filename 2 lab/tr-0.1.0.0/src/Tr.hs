...
makeSize str size 
    | length str /= 0 && length str < size  = str ++ replicate (size - length str) (last str)
    | otherwise  = str

replaceIfCont :: String -> String -> Char -> Char
replaceIfCont substring repSubstring getC  
    |cont == Nothing = getC
    |otherwise = let (Just val) = cont in val 
    where cont = lookup getC (zip substring repSubstring)