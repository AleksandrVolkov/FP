...
main = hspec $ describe "Testing tr" $ do
    ...
    describe "corner case" $
      it "aaaa" $
        tr' "" "b" "aaaa" `shouldBe` "aaaa"   -- тесты на корнер кейсы

    describe "corner case" $
      it "aaaa" $
        tr' "a" "" "aaaa" `shouldBe` "aaaa"   -- тесты на корнер кейсы   

    describe "corner case" $
      it "" $
        tr' "a" "b" "" `shouldBe` ""          -- тесты на корнер кейсы   
    ...
    describe "single delete" $ 
      it "c" $
        tr'' "c" "abcd" `shouldBe` "abd"

    describe "stream delete" $
      it "b" $
        tr'' "b" "bbbbbbb" `shouldBe` ""

    describe "delete" $
      it "123" $
        tr'' "123" "1234" `shouldBe` "4"
